...
const cucumberJsonDir = './test-results/cucumber-json'
...
function addScreenshots() {
  ...
  const screenshots = readdirRecursive(path.resolve(screenshotsDir)).filter(file => {
    return file.indexOf('.png') > -1 && file.includes('failed')
  })
...
function generateReport() {
  if (!fs.existsSync(cucumberJsonDir)) {
    ...
    report.generate({
      ...
      pageTitle: 'Instamotion Test Report',
      reportName: `Instamotion Report - ${new Date().toLocaleString()}`,
      hideMetadata: true
    })
  }
}